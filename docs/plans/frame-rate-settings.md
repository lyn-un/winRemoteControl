...
- 阶段一与阶段二的代码改造已实施：画质与帧率解耦、帧率菜单、协议与各处理层 144 FPS 上限、显示器刷新率能力发布、微秒级采集节拍、编码器动态帧率重建和 `stream_config_*` 诊断日志。
- 90～144 FPS 的实际效果仍需双机验证（60/90/120/144 Hz 显示器、硬件编码与 `libx264` 回退）；软件编码长期跑不满目标时再增加单独的策略上限。
- 控制中心 UI 已在 `theme-system` 合并后的菜单样式上实现。
