...
WebRTC H.264 decoder 输出 I420。`KWebRtcRemoteFrameProcessor` 将其转换为 Windows D3D 渲染所需的 BGRA，并采用“待处理帧只保留最新一张”的合并策略。它减少排队延迟，而不是传输错误；会话结束日志会输出 received、processed 和 coalesced 汇总。
